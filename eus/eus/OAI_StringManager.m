...
-(id)init {
    ...
    if (self = [super init]) {
        ...
        helpLaborCost = @"Nursing labor costs are based on Olympus 2013 Benchmarking Data of an average 3 hour total length of stay for EUS cases, and a 1:1 RN to patient ratio to compensate for other personnel costs (scheduler, tech support, etc.) at a rate of $35/hour. If physician is hospital employed, this figure also includes 1 MD hour/case at a rate of $200/hour.";
        ...
        helpMedicareFacilityFee = @"Estimated using 2013 Average HOPPS payment for CPT 43242. NOTE: Reduce reimbursement input if physician practice patterns will not typically include fine needle aspiration.  See reimbursement tab for details.  Source: 2013 Medicare Hospital Outpatient Final Rule.";
        
        helpNonMedicareFacilityFee = @"Non-Medicare Facility Fee per case is an approximation assuming Medicare Facility Fee per case increased  by 10 percent. This amount based on 2013 Medicare Fee Schedule for CPT 43242 ($927*1.1=$1019.70). See below for details if using \"other\" CPT code.";
        
        helpPhysicianFee = @"Only applicable if the EUS physician is hospital employed. Adjust with weighted averages as needed. Other CPT codes for reference: CPT 43232 = $264, CPT 43238 = $295. See reimbursement tab for details. Source: 2013 Medicare Physician Final Rule.\n\nWeighted Average:\nMedicare Fee Schedule based on an average of 65% of lowest physician CPT (264*.65=171.60) and 35% of the highest physician CPT code (428*.35=149.80).  Would equal $321.40";
        
        helpSecondProvider = @"Secondary providers include anesthesiologists, CRNAs, etc...";
        
        helpAnasthesiaPayment = @"National Average Medicare Reimbursement for anesthesia during endoscopy is based on CPT 00801 (anesthesia code): 5 Base Units and CPT 00810 (anesthesia for lower intestinal endoscopic procedures, endoscope introduced distal to duodenum): 5 Base Units and $21.92 2013 National anesthesia conversion factor = $110 payment. Only applicable if a secondary provider delivers anesthesia during EUS.  Source: 2013 Medicare Physician Final Rule.";
        
        helpPathology = @"National Average Medicare Reimbursement for Pathology is estimated based on the 2013 Medicare HOPPS Final Rule payment w/o geographic adjustment for APC343 (CPT 88305-26 Level IV surgical pathology): $25.  Assuming that the pathologist is a hospital employee, the most recent Medicare physician fee schedule for CPT 88305-26 (pathologist's interpretation): $24.  Exclude this amount if the pathologist is not a hospital employee. It is assumed one case will result in two biopsies:  ($25 + $24) x 2 = $99. Source: 2013 Medicare Physician and Hospital Outpatient Final Rules.";
        
        helpSurgery = @"National Average Medicare reimbursement for Surgery is based on the 2013 Medicare MS-DRG 328 (Stomach, Esophageal and Duodenal Proc w/o CC/MCC); Weight 1.3848, multiplied by 2013 National Inpatient Prospective Payment System Average Base rate excluding capital add-on ($5,209.74) = Payment $7,709. Source: 2013 Medicare Acute Inpatient PPS Final Rule.    ";
        
        helpAdmissions = @"National average (wage index greater than one) of the most common GI DRG rates is calculated by using the national adjusted full update standardized labor, non-labor and capital amounts. Source: MM8041 Revised January 4, 2013. Customers are encouraged to enter site-specific estimates based on claims data.";
        
        helpOncology = @"National Average Medicare Inpatient Reimbursement for Oncology is estimated based on the 2013 Medicare MS-DRG 484 (Chemotherapy w/o acute leukemia as secondary diagnosis w/o CC/MCC): Weight (1.9939) multiplied by 2013 National Inpatient Prospective Payment System Average Base rate excluding capital add-on ($5,209.74) = Payment $10,388. Source: 2013 Medicare Acute Inpatient PPS Final Rule.";
        
        helpRadiology = @"National Average Medicare Reimbursement for Radiology is estimated based on the average APC 276 (Level I Digestive Radiology) payment - $81.  Source: 2013 Medicare Hospital Outpatient Final Rule.";
        
        helpAnesthesia = @"National Average Medicare Reimbursement for Anesthesia is estimated based on the 2013 Medicare Physician fee schedule for CPT 00790 (anesthesia for intraperitoneal procedures in upper abdomen incl. laparoscopy). 7 base units multiplied by 2013 National anesthesia conversion factor of $21.52 = Payment of $154.  Source: 2013 Medicare Physician Final Rule.";
        ...
        [assumptionsArray addObject:[[NSArray alloc] initWithObjects:@"Labor costs per case", @"$105 (nursing)", @"Nursing labor costs are based on Olympus 2013 Benchmarking Data of an average 3 hour total length of stay for EUS cases, and a 1:1 RN to patient ratio to compensate for other personnel costs (scheduler, tech support, etc.) at a rate of $35/hour. If physician is hospital employed, this figure also includes 1 MD hour/case at a rate of $200/hour.",  nil]];
        ...
        [assumptionsArray addObject:[[NSArray alloc] initWithObjects:@"Medicare Facility Fee per case (does not include modifiers)", @"$927", @"Estimated using 2013 Average HOPPS payment for CPT 43242. NOTE: Reduce reimbursement input if physician practice patterns will not typically include fine needle aspiration.  See reimbursement tab for details.  Source: 2013 Medicare Hospital Outpatient Final Rule.",  nil]];
        [assumptionsArray addObject:[[NSArray alloc] initWithObjects:@"Non-Medicare Facility Fee per case", @"$1020", @"Non-Medicare Facility Fee per case is an approximation assuming Medicare Facility Fee per case increased  by 10 percent. This amount based on 2013 Medicare Fee Schedule for CPT 43242 ($927*1.1=$1019.70). See below for details if using \"other\" CPT code.",  nil]];
        [assumptionsArray addObject:[[NSArray alloc] initWithObjects:@"Physician Fee per case", @"$428", @"Only applicable if the EUS physician is hospital employed. Adjust with weighted averages as needed. Other CPT codes for reference: CPT 43232 = $264, CPT 43238 = $295. See reimbursement tab for details. Source: 2013 Medicare Physician Final Rule.\n\nWeighted Average:\nMedicare Fee Schedule based on an average of 65% of lowest physician CPT (264*.65=171.60) and 35% of the highest physician CPT code (428*.35=149.80).  Would equal $321.40",  nil]];
        [assumptionsArray addObject:[[NSArray alloc] initWithObjects:@"Anesthesia Fee per case", @"$110", @"National Average Medicare Reimbursement for anesthesia during endoscopy is based on CPT 00801 (anesthesia code): 5 Base Units and CPT 00810 (anesthesia for lower intestinal endoscopic procedures, endoscope introduced distal to duodenum): 5 Base Units and $21.92 2013 National anesthesia conversion factor = $110 payment. Only applicable if a secondary provider delivers anesthesia during EUS.  Source: 2013 Medicare Physician Final Rule.",  nil]];
        [assumptionsArray addObject:[[NSArray alloc] initWithObjects:@"Pathology Reimbursement", @"$99", @"National Average Medicare Reimbursement for Pathology is estimated based on the 2013 Medicare HOPPS Final Rule payment w/o geographic adjustment for APC343 (CPT 88305-26 Level IV surgical pathology): $25.  Assuming that the pathologist is a hospital employee, the most recent Medicare physician fee schedule for CPT 88305-26 (pathologist's interpretation): $24.  Exclude this amount if the pathologist is not a hospital employee. It is assumed one case will result in two biopsies:  ($25 + $24) x 2 = $99. Source: 2013 Medicare Physician and Hospital Outpatient Final Rules.",  nil]];
        [assumptionsArray addObject:[[NSArray alloc] initWithObjects:@"Surgery Reimbursement", @"$7,709", @"National Average Medicare reimbursement for Surgery is based on the 2013 Medicare MS-DRG 328 (Stomach, Esophageal and Duodenal Proc w/o CC/MCC); Weight 1.3848, multiplied by 2013 National Inpatient Prospective Payment System Average Base rate excluding capital add-on ($5,209.74) = Payment $7,709. Source: 2013 Medicare Acute Inpatient PPS Final Rule.  ",  nil]];
        [assumptionsArray addObject:[[NSArray alloc] initWithObjects:@"Admissions Reimbursement", @"$5,774", @"National average (wage index greater than one) of the most common GI DRG rates is calculated by using the national adjusted full update standardized labor, non-labor and capital amounts. Source: MM8041 Revised January 4, 2013. Customers are encouraged to enter site-specific estimates based on claims data.",  nil]];
        [assumptionsArray addObject:[[NSArray alloc] initWithObjects:@"Oncology Reimbursement", @"$10,388", @"National Average Medicare Inpatient Reimbursement for Oncology is estimated based on the 2013 Medicare MS-DRG 484 (Chemotherapy w/o acute leukemia as secondary diagnosis w/o CC/MCC): Weight (1.9939) multiplied by 2013 National Inpatient Prospective Payment System Average Base rate excluding capital add-on ($5,209.74) = Payment $10,388. Source: 2013 Medicare Acute Inpatient PPS Final Rule. ",  nil]];
        [assumptionsArray addObject:[[NSArray alloc] initWithObjects:@"Radiology Reimbursment", @"$81", @"National Average Medicare Reimbursement for Radiology is estimated based on the average APC 276 (Level I Digestive Radiology) payment - $81.  Source: 2013 Medicare Hospital Outpatient Final Rule.",  nil]];
        [assumptionsArray addObject:[[NSArray alloc] initWithObjects:@"Anesthesia - Non Endoscopy Reimbursement", @"$154", @"National Average Medicare Reimbursement for Anesthesia is estimated based on the 2013 Medicare Physician fee schedule for CPT 00790 (anesthesia for intraperitoneal procedures in upper abdomen incl. laparoscopy). 7 base units multiplied by 2013 National anesthesia conversion factor of $21.52 = Payment of $154.  Source: 2013 Medicare Physician Final Rule.",  nil]];
        ...
        [reimbursementArray addObject:[[NSArray alloc] initWithObjects:@"43232", @"Esophagoscopy, rigid or flexible; with transendoscopic ultrasound-guided intramural or transmural fine needle aspiration/biopsy(s)", @"$264", @"$264", @"$927", @"$520", nil]];
        [reimbursementArray addObject:[[NSArray alloc] initWithObjects:@"43238", @"Upper gastrointestinal endoscopy with transendoscopic ultrasound-guided intramural or transmural fine needle aspiration/biopsy(s) esophagus (includes endoscopic ultrasound examination limited to the esophagus", @"$295", @"$295", @"$927", @"$520", nil]];
        [reimbursementArray addObject:[[NSArray alloc] initWithObjects:@"43242", @"Upper gastrointestinal endoscopy with transendoscopic ultrasound-guided intramural or transmural fine needle aspiration/biopsy(s) (includes endoscopic ultrasound examination of the esophagus. Stomach, and either the duodenum and/or jejunum as appropriate)", @"$428", @"$428", @"$927", @"$520", nil]];
        [reimbursementArray addObject:[[NSArray alloc] initWithObjects:@"Lower Gastrointestinal Procedures", nil]];
        [reimbursementArray addObject:[[NSArray alloc] initWithObjects:@"45342", @"Sigmoidoscopy, flexible; with transendoscopic ultrasound guided intramural or transmural fine needle aspiration/biopsy(s)", @"$243", @"$243", @"$716", @"$402", nil]];
        [reimbursementArray addObject:[[NSArray alloc] initWithObjects:@"45392", @"Colonoscopy, flexible; with transendoscopic ultrasound guided intramural or transmural fine needle aspiration/biopsy(s)", @"$384", @"$384", @"$691", @"$308", nil]];
        [reimbursementArray addObject:[[NSArray alloc] initWithObjects:@"Medicare Hospital Inpatient Payment:\nInpatient payment information not shown because the transendoscopic ultrasound-guided fine needle aspiration procedure will rarely, if ever, be the primary reason for a hospital admission.\n\nNote: Represents National Average Medicare Fees (Without Geographic Adjustment) Last Updated March 2013.  N/A signifies Medicare expects that rarely, if ever, will this procedure be performed in this setting.", nil]];
        [reimbursementArray addObject:[[NSArray alloc] initWithObjects:@"\n1 Sources:", @"\nCPT & Description: CPT® 2013 American Medical Association. Hospital/ASC rates:  January 2013 Update to Appendix B and Appendix AA, respectively, from the CMS website.  Physician Fees:  Physician fee calculation by Olympus America Inc. and/or its direct or indirect (through one or more intermediaries) parent companies, affiliates, subsidiaries. Physician rates calculated using conversion factor of $34.0230 and RVUs published in the CMS Physician Fee Schedule January 2013 release. CPT Copyright 2013 American Medical Association. All rights reserved. CPT is a registered trademark of the American Medical Association.  All rights reserved. Applicable FARS/DFARS apply to government use.", nil]];
