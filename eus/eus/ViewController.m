...
- (NSMutableDictionary*) getSectionProperties : (int) s {
    ...
    if (s==0) {
        ...
        [table1Data addObject:[[NSArray alloc] initWithObjects: @"", @"", nil]];
        [table1Data addObject:[[NSArray alloc] initWithObjects: @"Will the physician performing EUS be facility employed?", @"Tabs", nil]];
        [table1Data addObject:[[NSArray alloc] initWithObjects: @"Labor costs per case", @"$105", nil]];
        ...
        [table1Data addObject:[[NSArray alloc] initWithObjects: @"Pathology", @"35%", @"$99", nil]];
        [table1Data addObject:[[NSArray alloc] initWithObjects: @"Surgery", @"2%", @"$7,709", nil]];
        [table1Data addObject:[[NSArray alloc] initWithObjects: @"Admissions", @"8%", @"$5,774", nil]];
        [table1Data addObject:[[NSArray alloc] initWithObjects: @"Oncology", @"2%", @"$10,388", nil]];
        [table1Data addObject:[[NSArray alloc] initWithObjects: @"Radiology", @"5%", @"$81", nil]];
        [table1Data addObject:[[NSArray alloc] initWithObjects: @"Anesthesia - Non Endoscopy", @"2%", @"$154", nil]];
...
- (void) buildSections : (NSMutableDictionary* ) sectionProperties : (UIView* ) sectionWrapper {
    ...
    if ([sectionTitle isEqualToString:@"Equipment"]) {
        ...
        for(int t=0; t<allTables.count; t++) {
            ...
            float tableWidth = [self getTableWidth:thisTableData:sectionTitle];
        ...
        if ([sectionTitle isEqualToString:@"Equipment"]) {
            ...
            UITextView* tvAdditionalEquip = [[UITextView alloc] initWithFrame:CGRectMake(10.0, lastTableFrame.origin.y + lastTableFrame.size.height + 50.0, maxTableWidth, 75.0)];
        ...
        float tableWidth = [self getTableWidth:table1StoredData:sectionTitle];
...
- (float) getTableWidth : (NSArray* ) thisTableData : (NSString*) strSection {
    ...
    for(int r=0; r<thisTableData.count; r++) {
        ...
        for(int x=0; x<thisTableRowData.count; x++) {
            ...
            if (![strSection isEqualToString:@"Equipment"]) { 
                if (thisRowItemSize.width < 100.0) {
                    thisRowItemSize.width = 140.0;
                }
            } else {
                if (thisRowItemSize.width < 100.0) {
                    thisRowItemSize.width = 180.0;
                }
            }
        ...
        if (r==0) {
            ...
            for(int d=0; d<thisRowData.count; d++) {
                ...
                if (d==0) {
                    ...
                    if ([thisCellLabel isEqualToString:@"Will the physician performing EUS be facility employed?"] || [thisCellLabel isEqualToString:@"Will a secondary provider deliver anesthesia during EUS cases?"]) {
                        
                        OAI_Switch* scSwitch = [[OAI_Switch alloc] initWithItems:[[NSArray alloc] initWithObjects:@"Yes", @"No", nil]];
                        [scSwitch setFrame:CGRectMake(dataX, dataY, 100.0, 30.0)];
                        
                        scSwitch.elementName = thisCellLabel;
                        [scSwitch addTarget:self action:@selector(valueChange:) forControlEvents:UIControlEventValueChanged];
                        [scSwitch setSelectedSegmentIndex:1];
                        
                        if ([thisCellLabel isEqualToString:@"Will the physician performing EUS be facility employed?"]) {
                            scSwitch.tag = 501;
                        } else {
                            scSwitch.tag = 502;
                        }
...
- (void) showInstructions : (OAI_InfoButton*) btnInfo  {
    ...
    NSArray* arrMySubviews = [vInstructions subviews];
    
    if (arrMySubviews.count > 0) {
        for(UIView* thisObj in arrMySubviews) {
...
- (void)itemSelected:(NSString *)thisItem {
    ...
    float medicareFee;
    float nonMedicareFee; 
    if ([strThisFacility isEqualToString:@"ASC"]) {
        medicareFee = 520;
    } else {
        medicareFee = 927;
    }
    nonMedicareFee = medicareFee*1.1;
    
    for(int i=0; i<placedElements.count; i++) {
        
        if ([[placedElements objectAtIndex:i] isMemberOfClass:[OAI_TextField class]]) {
         
            OAI_TextField* txtThisTextField = (OAI_TextField*)[placedElements objectAtIndex:i];
            NSString* strThisTextLabel = txtThisTextField.elementName;
            
            if ([strThisTextLabel rangeOfString:@"Medicare Facility Fee per case (does not include modifiers)"].location !=NSNotFound) {
                
                txtThisTextField.text = txtThisTextField.text = [NSString stringWithFormat:@"$%.0f", medicareFee];
            } else if ([strThisTextLabel rangeOfString:@"Non-Medicare Facility Fee per case"].location !=NSNotFound) {
                txtThisTextField.text = [NSString stringWithFormat:@"$%.0f", nonMedicareFee];
            }
        }
    }
...
- (void) valueChange : (OAI_Switch*) control {
    
    NSString* strSwitchName = control.elementName;
    
    if (control.tag == 501) {
        if ([control selectedSegmentIndex] == 0) {
            laborCost = 305;
            doctorReimbursement = 428;
        
        } else {
            laborCost =  105;
            doctorReimbursement = 0;
        }
    
    } else if (control.tag == 502) { 
        
        if ([control selectedSegmentIndex] == 0) {
            anesthesiaReimbursement = 110;
        } else { 
            anesthesiaReimbursement = 0;
        }
    }


    
    if ([strSwitchName isEqualToString:@"Will the physician performing EUS be facility employed?"]) {
        ...
        for(int i=0; i<placedElements.count; i++) {
            ...
            if ([[placedElements objectAtIndex:i] isMemberOfClass:[OAI_TextField class]]) {
                ...
                OAI_TextField* txtThisField = (OAI_TextField*)[placedElements objectAtIndex:i];
                NSString* strThisTextLabel = txtThisField.elementName;
                
                if ([strThisTextLabel rangeOfString:@"Labor costs per case"].location !=NSNotFound) {
                    txtThisField.text = [NSString stringWithFormat:@"$%.0f", laborCost];
                } else if ([strThisTextLabel rangeOfString:@"Physician Fee per case"].location !=NSNotFound) {
                    txtThisField.text = [NSString stringWithFormat:@"$%.0f", doctorReimbursement];
                }
            }
        }
        
    } else if ([strSwitchName rangeOfString:@"Will a secondary provider deliver"].location !=NSNotFound) {
        ...
        for(int i=0; i<placedElements.count; i++) {
            ...
            if ([[placedElements objectAtIndex:i] isMemberOfClass:[OAI_TextField class]]) {
                ...
                if ([strThisTextLabel rangeOfString:@"Anesthesia reimbursement"].location !=NSNotFound) {
                    txtThisField.text = [NSString stringWithFormat:@"$%.0f", anesthesiaReimbursement];
                }
            }
        }
...
- (void) checkKeyboardConflict {
    ...
    float firstResponderY;
    
    if ([vFirstResponder isMemberOfClass:[OAI_TextField class]]) {
        vParent = vFirstResponder.superview;
        firstResponderY = vParent.frame.origin.y + vParent.frame.size.height + 150.0;
        isTextField = YES;
    } else {
        firstResponderY = vFirstResponder.frame.origin.y + vFirstResponder.frame.size.height;
    }
    
    
    if (firstResponderY > myKeyboardFrame.origin.y) {
        
        float objDiff;
        
        //get the difference between the two origins
        if (!isTextField) { 
            objDiff = (vFirstResponder.frame.origin.y - myKeyboardFrame.origin.y) + (vFirstResponder.frame.size.height + 150.0);
        } else {
            objDiff = (vParent.frame.origin.y - myKeyboardFrame.origin.y) + (vParent.frame.size.height + 150.0);
        }
...
- (void) calculate {
    ...
    [scCalculatorSections setSelectedSegmentIndex:-1];
    ...
    for(NSString* thisKey in dataElements) {
        ...
        if (![thisElementType isEqualToString:@"String"]) {
            ...
            //check to make sure the required entries have data
    ...
    if(!isValidData) {
        ...
        //get anticipated patient growth (%)
        NSDictionary* dataSet_PatientGrowth = [dataElements objectForKey:@"Anticipated annual increase in EUS caseload_1"];
        NSString* strPatientGrowth = [dataSet_PatientGrowth objectForKey:@"Element Value"];
        NSString* strGrowthNum = [strPatientGrowth substringWithRange:NSMakeRange(0, 2)];
        float patientGrowth = [strGrowthNum floatValue]/100;
        ...
        float patientsPerWeekYear2 = patientsPerWeekYear1 + (patientsPerWeekYear1*patientGrowth);
        float patientsPerWeekYear3 = patientsPerWeekYear2 + (patientsPerWeekYear2*patientGrowth);
        ...
        //clean and convert
        strMedicareFacilityFeeValue = [self stripDollarSign:strMedicareFacilityFeeValue];
        
        if ([strMedicareFacilityFeeValue rangeOfString:@"."].location != NSNotFound) { 
            strMedicareFacilityFeeValue = [self stripDecimalPoints:strMedicareFacilityFeeValue];
        }
        
        
        float medicareFacilityFee = [strMedicareFacilityFeeValue floatValue];
        
        //[[self stripNonNumericCharacters:strMedicareFacilityFeeValue] floatValue];
        ...
        //facility non medicare fee per case
        ...
        //clean and convert
        strNonMedicareFacilityFeeValue = [self stripDollarSign:strNonMedicareFacilityFeeValue];
        
        if ([strNonMedicareFacilityFeeValue rangeOfString:@"."].location != NSNotFound) {
            strNonMedicareFacilityFeeValue = [self stripDecimalPoints:strNonMedicareFacilityFeeValue];
        }
        
        float nonMedicareFacilityFee = [strNonMedicareFacilityFeeValue floatValue];
        ...
        //formula
        physicianFeeYear1 = patientsAnnualYear1*doctorReimbursement;
        physicianFeeYear2 = patientsAnnualYear2*doctorReimbursement;
        physicianFeeYear3 = patientsAnnualYear3*doctorReimbursement;
        ...
        //formula
        anesthesiaFeeYear1 = patientsAnnualYear1*anesthesiaReimbursement;
        anesthesiaFeeYear2 = patientsAnnualYear2*anesthesiaReimbursement;
        anesthesiaFeeYear3 = patientsAnnualYear3*anesthesiaReimbursement;
        ...
        //get the value
        NSString* strEUME1_Value = [dataSet_EUME1_Value objectForKey:@"Element Value"];
        //strip the dollar sign from it
        strEUME1_Value = [self stripDollarSign:strEUME1_Value];
        //strip decimal points
        strEUME1_Value = [self stripDecimalPoints:strEUME1_Value];
        
        NSNumber* EUME1_Num = [numForm numberFromString:strEUME1_Value];
        float EUME1_Total = EUME1_Quantity * [EUME1_Num floatValue];
        ...
        //get the value
        NSString* strGFUC140PAL5_Value = [dataSet_GFUC140PAL5_Value objectForKey:@"Element Value"];
        //strip the dollar sign from it
        strGFUC140PAL5_Value = [self stripDollarSign:strGFUC140PAL5_Value];
        //strip decimal points
        strGFUC140PAL5_Value = [self stripDecimalPoints:strGFUC140PAL5_Value];
        
        NSNumber* GFUC140PAL5_Num = [numForm numberFromString:strGFUC140PAL5_Value];
        float GFUC140PAL5_Total = GFUC140PAL5_Quantity * [GFUC140PAL5_Num floatValue];
        
        //GF-UCT140-AL5
        NSDictionary* dataSet_GFUCT140AL5_Quantity = [dataElements objectForKey:@"GF-UCT140-AL5_1"];
        NSDictionary* dataSet_GFUCT140AL5_Value = [dataElements objectForKey:@"GF-UCT140-AL5_2"];
        
        int GFUCT140AL5_Quantity = [[dataSet_GFUCT140AL5_Quantity objectForKey:@"Element Value"] intValue];
        //get the value
        NSString* strGFUCT140AL5_Value = [dataSet_GFUCT140AL5_Value objectForKey:@"Element Value"];
        //strip the dollar sign from it
        strGFUCT140AL5_Value = [self stripDollarSign:strGFUCT140AL5_Value];
        //strip decimal points
        strGFUCT140AL5_Value = [self stripDecimalPoints:strGFUCT140AL5_Value];
        ...
        //get the value
        NSString* strGFUCT180_Value = [dataSet_GFUCT180_Value objectForKey:@"Element Value"];
        //strip the dollar sign from it
        strGFUCT180_Value = [self stripDollarSign:strGFUCT180_Value];
        //strip decimal points
        strGFUCT180_Value = [self stripDecimalPoints:strGFUCT180_Value];
        
        NSNumber* GFUCT180_Num = [numForm numberFromString:strGFUCT180_Value];
        float GFUCT180_Total = GFUCT180_Quantity * [GFUCT180_Num floatValue];
        ...
        //get the value
        NSString* strGFUE160_Value = [dataSet_GFUE160_Value objectForKey:@"Element Value"];
        //strip the dollar sign from it
        strGFUE160_Value = [self stripDollarSign:strGFUE160_Value];
        //strip decimal points
        strGFUE160_Value = [self stripDecimalPoints:strGFUE160_Value];
        
        NSNumber* GFUE160_Num = [numForm numberFromString:strGFUE160_Value];
        ...
        //get the value
        NSString* strSSDAlpha5_Value = [dataSet_SSDAlpha5_Value objectForKey:@"Element Value"];
        //strip the dollar sign from it
        strSSDAlpha5_Value = [self stripDollarSign:strSSDAlpha5_Value];
        //strip decimal points
        strSSDAlpha5_Value = [self stripDecimalPoints:strSSDAlpha5_Value];
        
        NSNumber* SSDAlpha5_Num = [numForm numberFromString:strSSDAlpha5_Value];
        float SSDAlpha5_Total = SSDAlpha5_Quantity * [SSDAlpha5_Num floatValue];
        
        //get additional equipment
        //SSD-Alpha 10
        
        NSDictionary* dataSet_Additional = [dataElements objectForKey:@"Additional Equipment_1"];
        
        //get the value
        NSString* strAdditional_Value = [dataSet_Additional objectForKey:@"Element Value"];
        //strip the dollar sign from it
        strAdditional_Value = [self stripDollarSign:strAdditional_Value];
        //strip decimal points
        strAdditional_Value = [self stripDecimalPoints:strAdditional_Value];
        
        NSNumber* strAdditional_Num = [numForm numberFromString:strAdditional_Value];
        ...
        NSDictionary* dataSet_LaborCost = [dataElements objectForKey:@"Labor costs per case_1"];
        NSString* strLaborCost = [dataSet_LaborCost objectForKey:@"Element Value"];
        
        //strip the dollar sign from it
        strLaborCost = [self stripDollarSign:strLaborCost];
        //strip decimal points if there is one
        if ([strLaborCost rangeOfString:@"."].location != NSNotFound) { 
        ...
        NSString* strServiceCost = [dataSet_ServiceCost objectForKey:@"Element Value"];
        
        //strip and clean the service cost
        
        strServiceCost = [self stripDollarSign:strServiceCost];
        strServiceCost = [self stripCommasFromNumbers:strServiceCost];
        //get service cost
        
        float serviceCost = [strServiceCost floatValue];
        ...
        NSString* strOtherVariableCost = [dataSet_VariableCostPerPatient objectForKey:@"Element Value"];
        
        //strip the dollar sign from it
        strOtherVariableCost = [self stripDollarSign:strOtherVariableCost];
        
        //strip decimal points if there is one
        if ([strOtherVariableCost rangeOfString:@"."].location != NSNotFound) {
            strOtherVariableCost = [self stripDecimalPoints:strOtherVariableCost];
        }
        
        //strip any commas
        strOtherVariableCost = [self stripCommasFromNumbers:strOtherVariableCost];
        
        //get the cost
        float variableCostPerPatient = [strOtherVariableCost floatValue];
...
- (NSString*) stripCommasFromNumbers : (NSString*) stringToStrip {
    
    NSString* newString = [stringToStrip stringByReplacingOccurrencesOfString:@"," withString:@""];
    return newString;
}
...
- (void) toggleEmailSetUp {
    ...
    if (vMailOptionsFrame.origin.y == 40.0) {
        vMailOptionsFrame.origin.y = -240.0;
    } else {
...
- (void) sendEmail : (UIButton*) btn {
    ...
    if (!isValid) {
        ...
        [emailBody appendString:@"<div><table style=\"width: 100%;border-top:1px solid #e5eff8; border-right:1px solid #e5eff8;\"><thead><tr style=\"background:#666; height: 30px;\"><th></th><th style=\"color:#eee;\">Year 1</th><th style=\"color:#eee;\">Year 2</th><th style=\"color:#eee;\">Year 3</th></tr></thead><tbody>"];
